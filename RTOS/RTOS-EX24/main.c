 ...
 * terminal.
 * 
 * Here the clearcountonexit argument of the tasknotifytake is set to TRUE which basically clears the notification value of the
 * task if any notification is received.
 * 
 * Interrupt events that occur between calls to ulTaskNotifyTake are latched in the task’s notification value,
 * and calls to ulTaskNotifyTake() will return immediately if the calling task already has notifications pending.
 * 
 * @version 0.1
 * @date 2022-06-05
 * 
...
{
    uint32_t EventToProcess;

    for(;;)
    {
        EventToProcess = ulTaskNotifyTake(pdTRUE,pdMS_TO_TICKS(750));
